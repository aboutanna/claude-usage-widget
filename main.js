...
const BASE_EXPANDED_SIZE = { width: 260, height: 188 };
...
function getExpandedSize() {
  return {
    width: BASE_EXPANDED_SIZE.width,
    height: BASE_EXPANDED_SIZE.height + (overflowActive ? OVERFLOW_HEIGHT : 0),
  };
}
...
function expandWidget() {
  ...
  const target = lastExpandedBounds || { x: 100, y: 100, width: size.width, height: size.height };
  const wa = getWorkAreaFor(target.x + size.width / 2, target.y + size.height / 2);

  let x = target.x;
  if (currentEdge === 'left') x = wa.x;
  if (currentEdge === 'right') x = wa.x + wa.width - size.width;

  const y = Math.min(Math.max(target.y, wa.y), wa.y + wa.height - size.height);
...
function setOverflowActive(active) {
  ...
  const wa = getWorkAreaFor(bounds.x + bounds.width / 2, bounds.y + bounds.height / 2);
  const newY = Math.max(bounds.y - sign * OVERFLOW_HEIGHT, wa.y);
  const newHeight = bounds.height + sign * OVERFLOW_HEIGHT;

  widgetWindow.setBounds({ x: bounds.x, y: newY, width: bounds.width, height: newHeight }, true);
...
function createWidgetWindow() {
  ...
  const startX = typeof cfg.x === 'number' ? cfg.x : width - size.width - 24;
  ...
  lastExpandedBounds = { x: startX, y: startY, width: size.width, height: size.height };
  currentEdge = computeEdge(startX, size.width, startY);
  ...
  widgetWindow.on('moved', () => {
    ...
    currentEdge = computeEdge(x, BASE_EXPANDED_SIZE.width, y);
